...
> 2026-08-28에 추가했다 - 라이브 벤치마크 재조사로 실측 발견한 오매칭 패턴에
> 대응한 것으로, [성능/품질 개선 기록](#성능품질-개선-기록) 참고.
...
6. **현재(08-28~)**: raw 모드 전환(3번 항목) 이후 100개 라이브 벤치마크로 실측 검증. raw 모드가 구조적 필터를 건너뛸 때 수량/용량이 다른 후보나 브랜드가 다른 후보가 그대로 채택되는 회귀를 발견해, DeepSeek 의미 재필터링 → 수량 가드 → 브랜드 우선순위 재정렬 → 다나와(신규 보조 검색 소스) 순으로 안전망을 추가했다. 08-31에는 여기서 한 단계 더 나아가 Baseline·1st Proposed·2nd Proposed(raw 모드) 세 아키텍처 전체를 동일한 100개 쿼리로 라이브 3웨이 비교했다(자세한 수치는 [성능/품질 개선 기록](#성능품질-개선-기록) 참고)
...
- **DeepSeek 의미 재필터링이 수량/용량이 다른 후보까지 통과시킴**(라이브 벤치마크 재조사 실측 - "메로나 20개입" 검색에 "메로나 30개입"이 통과됨, DeepSeek 프롬프트가 "같은 품목의 다른 묶음은 포함"하라고 지시하는 게 원인): DeepSeek이 고른 인덱스에 `model_or_quantity_conflict`(결정적 규칙, LLM 편차 없음)를 한 번 더 적용해 수량이 명시적으로 다른 후보를 최종 제외
...
- **라이브 검색 API 결과가 재실행마다 흔들려 회귀 검증에 노이즈가 섞임**(재실행 결과가 코드 변경 효과와 무관하게 흔들리는 걸 실측 - 11번가 검색 API 결과 자체가 시간차로 달라지고 DeepSeek 재채점도 완전히 결정적이지 않아 발생): 라이브 검색 API를 쓰는 이 파이프라인의 회귀 검증을 대규모 재실행이 아니라 **문제가 재현된 고정 질의를 수정 전/후로 직접 대조**하는 방식으로 전환
...
- **(08-28)** raw 모드 전환 후 라이브 벤치마크로 드러난 오매칭을 브랜드/수량·용량/카테고리/부속품 유형으로 분해해 각각 안전망을 추가(DeepSeek 재필터링의 수량 가드, 브랜드 우선순위 재정렬, 다나와 보조 검증) - 검색 API 결과 변동 노이즈 때문에 대규모 재실행으로는 개선을 깔끔하게 증명하기 어려워, 문제가 재현된 고정 질의로 직접 대조해 검증
...
- **(09-01)** 위 3웨이 비교에서 1st Proposed가 raw 모드보다 Precision이 근소 우위(70.0% vs 66.0%)였던 걸 근거로, "구조적 필터가 통과율 낮아 DeepSeek 재필터링을 타는 애매한 경우에만 judge(Qwen)를 예외적으로 호출"하는 **하이브리드 게이트를 시험 구현하고 100개 쿼리로 실측 검증**했으나 가설이 기각됐다 - judge가 개입한 42건 중 개선 0건, 오히려 5건이 raw에서는 맞았는데 judge 개입 후 틀리게 바뀌었고(전체 Precision 66.0% → 61.0%로 하락, 레이턴시도 +16% 증가), judge가 검색 API의 관련도순 1위보다 상품명의 세부 조건(예: "초보자용", "오버핏") 일치를 오히려 덜 반영하는 경향이 확인됐다. 원인은 검색 소스 자체(11번가 관련도순)가 이미 충분히 신뢰할 만해서, 판단을 추가하는 게 득보다 실이 크다는 것 - **개조를 원상복구하고 raw 모드(judge 무조건 스킵)를 그대로 유지**하기로 결정. "성능이 비슷하면 판단을 더 넣는 쪽이 낫다"는 직관이 항상 맞지는 않는다는 실측 사례로 남긴다
...
**2026-09-01**
- raw 모드에 "필터 통과율이 낮은 애매한 쿼리에만 judge(Qwen)를 예외적으로 호출"하는 하이브리드 게이트(`needs_judge_review` 플래그 + `_skip_judge_unless_review_needed`)를 시험 구현, 100개 쿼리로 실측 검증
- 검증 결과 가설 기각 - judge 개입 42건 중 개선 0건, 악화 5건(전체 Precision 66.0%→61.0%, 레이턴시 +16%) - 개조를 원상복구하고 raw 모드(judge 무조건 스킵)를 그대로 유지(자세한 내용은 위 [성능/품질 개선 기록](#성능품질-개선-기록) 09-01 항목 참고)
...
- 정량 평가 체계는 100개 라이브 벤치마크(`backend/benchmark/`, 2026-08-31 3웨이 비교로 정착)로 도입됐지만 상시 회귀 하네스는 아니고, 라이브 검색 API를 쓰는 구조상 재실행 결과 자체가 시간에 따라 흔들려(검색 결과 변동·LLM 재채점 비결정성) 대규모 반복 실행으로 개선폭을 깔끔하게 증명하기 어렵다는 한계를 실측으로 확인했음 - 회귀 검증은 고정 질의 직접 대조 방식이 더 신뢰도 높음
...
- 라이브 벤치마크 재조사로 드러난 진짜 오매칭 중 브랜드/수량 불일치는 개선했지만, 카테고리 불일치(예: 파우더 검색에 팩트가 채택되는 등 세부 상품 형태 차이)와 본품/부속품 혼동은 이번 세션 범위에서 다루지 않고 남겨둠
