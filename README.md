...
> 여러 모델이 후보를 각자 "제안"하는 구조가 아니라, refine(HCX) → 검색 →
...
| 그라운딩 검증 | DeepSeek(challenge 단계) - 관련성 검증을 통과한 상위 후보를 의미 기반으로 다시 검증(`verified`/`challenge_note`), judge 프롬프트에도 "[검증 실패: ...]" 표시로 그대로 전달돼 최종 선택에 반영됨. 상위 후보가 전부 미검증으로 나오면 가격 보정 검색을 강제로 켠 채 파이프라인을 한 번 더(최대 1회) 실행 |
...
| 2026-08-25 ~ 26 | 단발 검색 표본을 10 → 30개로 확대(가격대 다양성 확보 목적) · Qwen(DashScope) 쿼터 소진으로 "gpt" 슬롯(추천 Agent·질의 정제)을 임시로 HCX 호출로 전환 · **신뢰 신호 기반 의심 후보 후순위화 도입**: 동일 판매자 리스팅 수 · 시세(중앙값) 대비 파격 저가 · "미개봉"/"완납" 등 약정 의심 문구 3가지를 신호로 반영, 처음엔 프롬프트 경고 문구로만 얹었으나 HCX가 반복 무시하는 걸 실측(2회 재현)해 코드 레벨 후순위 정렬(`deprioritize_suspicious`)로 전환 · facet 드릴다운 경로의 관련성 가드 누락 발견·수정(8/24 도입분 - 케이스/무선마이크 등 액세서리가 그대로 추천되던 버그) · 액세서리 단어 블랙리스트(`ACCESSORY_TERMS`) 보강 · 신제품 등 검색어가 짧고 일반적일 때 가격 오름차순 정렬이 저가 액세서리로 표본을 채워 본품이 후보에서 아예 밀려나는 구조적 문제 발견, `sortCd="H"`(가격 내림차순) 보정 검색 도입(`most_candidates_look_like_accessories` 트리거) · **Google ADK를 단일 소스(11번가) 전용 8단계 `SequentialAgent`로 재도입**(refine → search → propose → filter_merge → extract_pages → challenge(DeepSeek 그라운딩 검증) → apply_challenge → judge, 상위 후보 전부 미검증이면 보정 검색 강제 후 1회 재실행) · HITL이 숫자 포함 검색어를 무조건 스킵하던 버그 수정 · AI 상세검색 facet에 검색어와 무관한 폰 브랜드가 섞이는 문제 수정 · 1단계 refine을 Qwen에서 HCX로 교체 · judge가 challenge 검증 결과를 못 보고 최종 추천을 고르던 문제 수정(두 단계를 잇는 `_candidates_with_verdicts` 추가, judge 프롬프트에 "[검증 실패: ...]" 표시) · challenge가 놓친 명백한 액세서리를 규칙 기반으로 한 번 더 검증하는 보강 · `_search_candidates`가 11번가 검색 실패(깨진 XML 등) 시 파이프라인 전체를 죽이던 문제 수정 · Supabase LLM 캐시 키를 `sb_publishable_...`(anon, RLS가 쓰기를 막음)에서 `sb_secret_...`(service_role)로 교체해 실제로 캐시가 동작하도록 수정(실측: 동일 질의 재요청이 18초 → 3.6초) · 액세서리 도배 감지를 키워드 트리거가 놓친 케이스까지 잡도록 DeepSeek 의미 기반 재확인(`looks_accessory_flooded`) 추가(키워드 트리거가 이미 걸렸으면 비용 절감을 위해 LLM 호출 생략) · 어디서도 안 쓰이던 `run_elevenst_only_debate_stream`(main.py가 실제로는 `adk_pipeline.run_stream`을 쓴 지 오래라 참조가 끊긴 상태였음), `search_categories`/`_search_elevenst_categories`(2026-08-20에 카테고리 축을 뺀 이후 죽어있던 코드), `schemas.py`의 미사용 모델 6종(`SearchResult`/`ChallengeVerdict`/`ChallengeResult`/`JudgeVerdict` - adk_pipeline이 자체 private 버전을 씀) 제거 · AI 상세검색 "제품분류" facet에 케이스/충전기/어댑터/스마트톡이 선택지로 뜨던 문제 수정(기존 필터는 라벨이 정확히 "카테고리"일 때만 걸렀는데 DeepSeek이 같은 축을 "제품분류"로 다르게 이름 붙이면 통과됐음 - 라벨이 아니라 옵션 값 자체를 보는 `_strip_accessory_options`로 교체) · **등급 편중(상위 등급만 매핑/추천되는 문제) 감지 및 3중 보정 추가**: "아이폰 17"/"아이폰 16"처럼 등급이 있는 상품을 검색하면 프로/프로맥스 등 상위 등급 매물이 압도적으로 많아 기본형이 sortCd=A/H 표본 250개까지 넓혀도 0건인 경우를 실측 확인. (1) DeepSeek이 표본에 다른 등급이 섞여 있는지 항상 판정하고(`deepseek.check_grade_mismatch` - 처음엔 "정확한 등급이 표본에 있으면 판정 자체를 스킵"했으나, 있어도 임베딩 랭킹이 등급 차이를 못 구분해(코사인 유사도 차 0.001 수준, 실측) 여전히 다른 등급이 1위로 뜨는 문제를 발견해 "정확한 등급 유무와 무관하게 등급 토큰 항상 식별"로 재설계) 없으면 카테고리에 맞는 보정 키워드(휴대폰류는 "자급제" 등, 하드코딩 아님)로 재검색해 병합, (2) 그 등급 토큰이 없는 후보를 관련도 랭킹 뒤에서 우대 정렬(`_prioritize_exact_grade`), (3) judge/추천 Agent 프롬프트에도 "[다른 등급]" 표시를 추가해 순서와 무관하게 자유롭게 고르는 LLM이 여전히 다른 등급을 선택하는 걸 막음. 판정 temperature=0으로 고정(기본값으로는 동일 입력에도 3회 중 1회꼴로 판정이 뒤집힘을 실측). judge 캐시 키에도 등급 토큰을 반영(안 그러면 프롬프트가 바뀌어도 등급 미반영 시절 캐시가 그대로 재사용됨) |
| 2026-08-27 | **LLM 호출 지점 재평가**: judge 단계가 관련성 필터를 통과한 후보가 정확히 1개일 때도 매번 Qwen을 호출하던 낭비를 발견해, 그 경우 LLM 호출 없이 후보를 즉시 확정하도록 수정(`_skip_judge_if_single_candidate`, 단 challenge에서 verified=False로 나온 유일 후보는 LLM 판단을 유지) · challenge(DeepSeek 2차 의미 검증) 단계를 규칙 기반으로 대체할 수 있는지 골든셋 50개(10카테고리×5) 라이브 실측으로 검증 - 가설(규칙 필터가 이미 정교해 challenge가 중복일 것)은 기각됨: 전체 후보 414건 중 24.6%(102건)가 verified=False였고 그중 64.7%(66건)는 규칙 사전으로는 전혀 못 잡는 순수 의미 판단("갤럭시 S25 FE/엣지/플러스"를 "갤럭시 S25"와 다른 모델로 구분, "삼성 냉장고 컴퓨터 보드"/정수기 모듈을 부품으로 판정 등) · 이 실측 과정에서 `Decision.verified`가 스키마 문서("challenge 불필요 시에도 True로 강제되어 None에는 안 걸림")와 달리 `_build_decision`이 verified 인자 자체를 안 받아 골든셋 43건 전량 None으로 나오던 버그 발견·수정 |
...
- **카테고리 데이터**: 조회하지 않는다 - 2026-08-20 "카테고리 축은 아예 다루지 않는다" 결정 이후 실제로 어디서도 안 쓰였던 `fetchers/elevenst.py::search_categories`(`option=Categories` 집계 조회)와 그 래퍼 `app/price_table.py::_search_elevenst_categories`를 2026-08-26에 제거했다. AI 상세검색의 "카테고리" facet은 대신 DeepSeek이 상품명에서 자체적으로 뽑아온 걸 의도적으로 걸러낸다(`app.debate.check_clarify_facets`)
...
2. **refine**: 대화체 질의만(`looks_conversational_query`) HCX에게 정제 요청, 이미 구체적이면 LLM 호출 자체를 건너뜀
...
- Supabase 기반 LLM 응답 캐시(`app/llm_cache.py`, 스키마 `supabase/llm_cache.sql`)는 2026-08-26부터 실제로 활성화됨 - 처음엔 `SUPABASE_KEY`가 `sb_publishable_...`(anon) 키라 RLS가 캐시 테이블 쓰기를 막아 조용히 no-op였는데, `sb_secret_...`(service_role) 키로 교체해 해결
