...
_ACCESSORY_INDICATOR_TOKENS = {
    ...
    # 2026-08-26, 사용자 리포트("아이폰 17 쳤을때 케이스가 뜨는데") 재현 중
    # 발견 - "필터"(카메라 렌즈 보호 필터)와 "스마트톡"(맥세이프 그립형
    # 액세서리)이 "아이폰 17" 검색 표본에 흔한데 기존 목록에 없어 안 잡혔다.
    "필터", "스마트톡",
    # "이어폰"/"헤드폰"/"헤드셋" 제거(2026-08-28, 실측 발견 - HITL 축 순서
    # 버그를 조사하다가 "이어폰" 검색 자체가 _looks_like_accessory에 걸려
    # "액세서리를 찾는 질의"로 오분류되는 걸 확인했다). 이 세 카테고리는
    # 케이스/충전기처럼 "다른 기기 없이는 무의미한 부속품"이 아니라 그
    # 자체로 완전히 독립된 본품 카테고리다 - 액세서리 지시어에 두면 인 안
    # 되는 게 아니라 되면 안 된다("이어폰용 이어팁"처럼 진짜 부속품은
    # 위의 "이어버드"/"이어훅"/"이어팁"이 이미 잡는다).
}
